...
bool check(char* word)
{
    ...
    for (int i = 0; i < length; i++)
    {
        ...
        // check if current character is within the alphabet. (a - z), if not return false.
       if (charIndex < 0 || charIndex >= 26) {
            return false;
        }

        // check if the current character is in the trie, if not return false.
        // children[] is a part of the node structure
    ...
    // check if found
    // this is outside the loop and cursor is pointing at the last character of the word.
    // if we are at the end
    bool wordFound = cursor->is_word;
